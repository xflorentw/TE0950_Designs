...
int main(int argc, char* argv[])
{
	...
	int32_t golden, dataIn;
	int16_t realTemp, imTemp;
	
	int errorCount=0;

    for(int i = 0; i < SAMPLES; i++) {
    	dataIn = ((int32_t*)in_bomapped)[i];
    	realTemp = (int16_t)((dataIn & 0xFFFF0000)>>16) + (int16_t)(dataIn & 0xFFFF);
    	imTemp = (int16_t)((dataIn & 0xFFFF0000)>>16) - (int16_t)(dataIn & 0xFFFF);
    	golden =  ((int32_t)(realTemp + imTemp) << 16) + (((int32_t)realTemp - (int32_t)imTemp)& 0xFFFF);
			
    	if(golden != ((int32_t*)out_bomapped)[i])
    	{
    		errorCount++;
    		printf("Output Error: Golden = %x Output = %x\n ",golden,((int32_t*)out_bomapped)[i]);
			printf("Input = %x, Real Temp = %x, imag Temp = %x\n\n",dataIn, realTemp, imTemp);
    	}
    }
	if (errorCount)
