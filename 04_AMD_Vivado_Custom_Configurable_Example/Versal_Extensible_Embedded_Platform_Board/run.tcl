...
proc create_root_design {currentDir design_name use_lpddr clk_options irqs use_aie} {
	...
	set_property -dict [list CONFIG.NUM_SI {0} CONFIG.NUM_MI {0} CONFIG.NUM_NSI {1} CONFIG.NUM_CLKS {0}  CONFIG.NUM_MCP {4} CONFIG.LOGO_FILE {data/noc_mc.png} CONFIG.MC_CHAN_REGION1 {DDR_LOW1}] [get_bd_cells noc_ddr4]
	...
	# Enable PL clock and reset in CIPS
	set_property -dict [list CONFIG.PS_PMC_CONFIG { PS_NUM_FABRIC_RESETS 1 PS_USE_PMCPL_CLK0 1 }] [get_bd_cells CIPS_0]
...
if { $use_aie eq "true" } {
	...
	\t Note: 
	\t --> PS IOs are enabled in the CIPS based on board configuration file
	...
	set_property USER_COMMENTS.comment0 {\t \t \t =============== >>>> An Example Versal Extensible Embedded Platform <<<< ===============
	\t Note: 
	\t --> PS IOs are enabled in the CIPS based on board configuration file} [current_bd_design]
}
